...
function setup(app, createPage, onRoute) {
  ...
  app.router = function(options) {
    ...
    routes.forEach(function(route) {
      ...
      if (isTransitional(pattern)) return transitional.push(route);
    ...
    transitional.forEach(function(route) {
      var method = route[0]
        , pattern = route[1]
        , callback = route[2]
        , from = pattern.from
        , to = pattern.to
        , forward = pattern.forward || callback.forward || route[2]
        , back = pattern.back || callback.back

      if (forward) addTransitional(method, to, from, forward)
      if (back) addTransitional(method, from, to, back)
    })
    ...
    function addTransitional(method, to, from, callback) {
      function fn(req, res, next) {
        ...
        console.log('hihihi', to , from, req.url)
