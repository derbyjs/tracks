...
function History(page) {
  ...
  if (winHistory.pushState) {
    ...
    this.push = this.replace = function(url){
...
function addListeners(history, page) {
  ...
  function onSubmit(e) {
    var target = e.target
      , _defaultPrevented = e.defaultPrevented;

    if (target.tagName.toLowerCase() === 'form') {
      if (!e.returnValue || target._forceSubmit || target.enctype === 'multipart/form-data') {
        return
      }

      // Prevent default form submit to try process it locally
      if (e.preventDefault)
        e.preventDefault();

      // Overriding preventDefault to see if other sources using it
      ...
      setTimeout(function () {
        ...
        // If submit was not prevented by other sources try to handle it locally
        if (!_defaultPrevented)
          history.push(
              target.action || winLocation.href,
              true,
              null,
              e
          );
        ...
        setTimeout(function () {
          ...
          // Fallback: if submit was not prevented by router - submit form to server
          if (!_defaultPrevented) {
            target._forceSubmit = true;
            target.submit();
          }
        }, 0);
